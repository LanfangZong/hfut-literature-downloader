...
async function main() {
  ...
  const initJs = `(
    async () => {
      ...
      window.__hfutLiteratureDownloaderBytes = new Uint8Array(ab);
      return {
        ok: r.ok,
        status: r.status,
        contentType: ct,
        size: window.__hfutLiteratureDownloaderBytes.length,
        url: location.href,
        head: Array.from(window.__hfutLiteratureDownloaderBytes.slice(0, 8))
      };
    }
  )()`;
  ...
  for (let start = 0; start < size; start += args.chunkSize) {
    ...
    const chunkJs = `(
      () => {
        const bytes = window.__hfutLiteratureDownloaderBytes.slice(${start}, ${end});
        let bin = "";
